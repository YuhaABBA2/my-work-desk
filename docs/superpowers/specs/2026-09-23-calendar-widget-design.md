...
- 대가: 그림이 한 덩어리라 **어느 날짜를 눌렀는지 알 수 없다.** 탭은 위젯 전체 하나뿐

## 위젯을 눌렀을 때

`https://my-work-desk.vercel.app/?d=<오늘 ISO>` 를 연다.
앱이 `?d=`를 읽어 그 날짜의 `openDayDialog(d)`를 띄운다 — **오늘 일정이 바로 펼쳐진다.**

앱은 현재 URL 파라미터를 전혀 읽지 않는다(`URLSearchParams` 사용처 0건).
`openDayDialog(dateIso)`는 이미 있으므로 진입점에서 파라미터를 읽어 호출하기만 하면 된다.
잘못된 값이면 무시하고 평소대로 뜬다.

### 기종별로 어디가 열리는가

| | 도착지 |
|---|---|
| 안드로이드 | 홈화면에 PWA를 깔아뒀으면 **PWA 앱**이 열린다 (Chrome WebAPK가 주소를 가로챈다). 아니면 브라우저 |
| 아이폰 | **사파리**가 열린다. iOS는 홈화면 웹앱이 주소를 가로채지 못한다 |

⚠️ **아이폰 제약**: iOS에서 홈화면에 추가한 웹앱은 사파리와 저장 공간이 분리돼 있다.
그래서 위젯을 눌러 사파리로 열리면 **로그인이 안 된 상태일 수 있다.**
애플이 그렇게 만들어 둔 것이라 우회할 방법이 없다 — 알고 쓰는 수밖에 없다.
...
4. `?d=2026-09-23`으로 앱을 열면 그 날짜 창이 뜬다. `?d=` 없거나 값이 이상하면 평소대로 뜬다
